...
describe('Problem 7 - mimeFromFilename() function', function () {
  ...
  test('correct mime type for MEPG extension', function () {
    expect(mimeFromFilename('data.mpeg')).toEqual('video/mpeg');
  });

  test('correct mime type for CSV extension', function () {
    expect(mimeFromFilename('data.csv')).toEqual('text/csv');
  });

  test('correct mime type for TTF extension', function () {
    expect(mimeFromFilename('data.ttf')).toEqual('font/ttf');
  });

  test('correct mime type for WOFF extension', function () {
    expect(mimeFromFilename('data.woff')).toEqual('font/woff');
  });

  test('correct mime type for ZIP extension', function () {
    expect(mimeFromFilename('data.zip')).toEqual('application/zip');
  });

  test('correct mime type for AVI extension', function () {
    expect(mimeFromFilename('data.avi')).toEqual('video/x-msvideo');
  });
  ...
  test('bare filename (no path) gives correct mime type', function () {
    expect(mimeFromFilename('index.html')).toEqual('text/html');
  });
